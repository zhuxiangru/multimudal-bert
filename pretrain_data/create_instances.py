...
flags.DEFINE_string("entity2id_file", None,
                    "The entity2id file that the BERT model was trained on.")
flags.DEFINE_string("image2id_file", None,
                    "The image2id file that the BERT model was trained on.")
...
flags.DEFINE_float(
    ...
    "maximum length.")
#vocab_words_size = 30521 # chinese character: 721

vocab_file = FLAGS.vocab_file
entity2id_file = FLAGS.entity2id_file
image2id_file = FLAGS.image2id_file

tmp_vocat_cnt = 0
with open(vocab_file, "r", encoding = "utf-8") as vocab_infile:
    for line in vocab_infile:
        tmp_vocat_cnt += 1
vocab_words_size = tmp_vocat_cnt
...
def create_training_instances(input_file, output_file, max_seq_length,
    ...
    # read entity mapping
    with open(entity2id_file, 'r') as fin:
        d_ent = {}
        fin.readline()
        while 1:
            l = fin.readline()
    ...
    with open(image2id_file, 'r') as fin:
        d_img = {}
        fin.readline()
        while 1:
            l = fin.readline()
    ...
    while i < document[0]:
        ...
        if i == document[0] - 1 or current_length >= target_seq_length:
            if current_chunk:
                ...
                # 原要求一个句子中的实体数目超过5个。可能是为了结果显著。这让很多语料无法参与训练
                #if len([x for x in entity if x > -1]) >= 5:
                if len([x for x in entity if x > -1]) >= 1:
